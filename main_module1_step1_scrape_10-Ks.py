...
save_dir = os.path.join(root_dir, 'pt_10-Ks')
if not os.path.exists(save_dir): os.makedirs(save_dir)
save_filepath_format = os.path.join(save_dir, 'Scraped_{}.csv')
...
if __name__ == '__main__':
    ...
    df = pd.read_csv(companies_filepath)[['Ticker', 'Name', 'CIK']]
    df.columns = ['Ticker', 'CompanyName', 'CIK']

    records, err_records = [], []    
    for form_type in ['10-K']:
        ...
        for (ticker, cik, company_name) in tqdm(df[['Ticker', 'CIK', 'CompanyName']].values):   
            base_url_filled = base_url.format(cik, form_type, end_date.replace('-', '')) # ticker or CIK
            soup = get_soup(base_url_filled) 
